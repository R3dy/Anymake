...
| `anymake-dispatch` | Hardened, host-agnostic sub-agent dispatch — the single chokepoint for spawning any agent. Wraps the host's `Agent`/`Task` primitive with pre-dispatch prompt assembly (WRITE THE FILE FIRST), mandatory post-dispatch deliverable verification, structured RETRY CONTEXT, and a dispatch log to `BOARD.md` | Whenever any agent needs to spawn another | "dispatch the planner", "spawn the worker", "re-dispatch with retry context", "invoke the validator" |
